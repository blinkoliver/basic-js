class VigenereCipheringMachine {
  ...
  getString(message, key, isDecrypt = false) {
    ...
    for (let i = 0, j = 0; i < message.length; i++) {
      ...
      let keyRow = this.alphabet
        .slice(keyIndex, this.alphabet.length)
        .concat(this.alphabet.slice(0, keyIndex));
      let messageIndex = (isDecrypt ? keyRow : this.alphabet).indexOf(
        message[i]
      );
